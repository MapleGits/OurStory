...
public class DumpQuests
{
  ...
  public DumpQuests(boolean update) throws Exception {
    this.update = update;
    this.quest = MapleDataProviderFactory.getDataProvider(new File(new StringBuilder().append(System.getProperty("net.sf.odinms.wzpath")).append("/Quest.wz").toString()));
    if (this.quest == null)
      this.hadError = true;
  }
  ...
  public void dumpQuests() throws Exception {
    if (!this.hadError) {
      ...
      PreparedStatement ps = this.con.prepareStatement("INSERT INTO wz_questdata(questid, name, autoStart, autoPreComplete, viewMedalItem, selectedSkillID, blocked, autoAccept, autoComplete) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
  ...
  public void dumpQuests(PreparedStatement psai, PreparedStatement psas, PreparedStatement psaq, PreparedStatement ps, PreparedStatement psr, PreparedStatement psq, PreparedStatement psa) throws Exception
  {
    ...
    for (MapleData qz : checkz.getChildren()) {
      this.id = Integer.parseInt(qz.getName());
      if ((!this.update) || (!doesExist(new StringBuilder().append("SELECT * FROM wz_questdata WHERE questid = ").append(this.id).toString())))
      {
        ps.setInt(1, this.id);
        for (int i = 0; i < 2; i++) {
          MapleData reqData = qz.getChildByPath(String.valueOf(i));
          if (reqData != null) {
            ...
            for (MapleData req : reqData.getChildren())
              if (MapleQuestRequirementType.getByWZName(req.getName()) != MapleQuestRequirementType.UNDEFINED)
              {
                ...
                StringBuilder intStore1 = new StringBuilder();
                StringBuilder intStore2 = new StringBuilder();
                List dataStore = new LinkedList();
                if (req.getName().equals("job")) {
                  List child = req.getChildren();
                  for (int x = 0; x < child.size(); x++)
                    dataStore.add(new Pair(Integer.valueOf(i), Integer.valueOf(MapleDataTool.getInt((MapleData)child.get(x), -1))));
                }
                else if (req.getName().equals("skill")) {
                  List child = req.getChildren();
                  for (int x = 0; x < child.size(); x++) {
                    MapleData childdata = (MapleData)child.get(x);
                    if (childdata != null)
                    {
                      dataStore.add(new Pair(Integer.valueOf(MapleDataTool.getInt(childdata.getChildByPath("id"), 0)), Integer.valueOf(MapleDataTool.getInt(childdata.getChildByPath("acquire"), 0))));
                    }
                  }
                } else if (req.getName().equals("quest")) {
                  List child = req.getChildren();
                  ...
                  for (int x = 0; x < child.size(); x++) {
                    ...
                    if (childdata != null)
                    {
                      dataStore.add(new Pair(Integer.valueOf(MapleDataTool.getInt(childdata.getChildByPath("id"), 0)), Integer.valueOf(MapleDataTool.getInt(childdata.getChildByPath("min"), 0))));
                    }
                  }
                } else if (req.getName().equals("pet")) {
                  List child = req.getChildren();
                  for (int x = 0; x < child.size(); x++) {
                    ...
                    if (childdata != null)
                    {
                      dataStore.add(new Pair(Integer.valueOf(i), Integer.valueOf(MapleDataTool.getInt(childdata.getChildByPath("id"), 0))));
                    }
                  }
                }
          ...
          MapleData actData = actz.getChildByPath(new StringBuilder().append(this.id).append("/").append(i).toString());
          if (actData != null) {
            ...
            for (MapleData act : actData.getChildren())
              if (MapleQuestActionType.getByWZName(act.getName()) != MapleQuestActionType.UNDEFINED)
              {
                ...
                if ((act.getName().equals("sp")) || (act.getName().equals("skill"))) {
                  ...
                  while (act.getChildByPath(new StringBuilder().append(index).append("/job").toString()) != null) {
                    for (MapleData d : act.getChildByPath(new StringBuilder().append(index).append("/job").toString())) {
                      if (applicableJobs.length() > 0) {
                        applicableJobs.append(", ");
                      }
                      applicableJobs.append(MapleDataTool.getInt(d, 0));
        ...
        System.out.println(new StringBuilder().append("Added quest: ").append(this.id).toString());
      }
    }
    System.out.println("Done wz_questdata...");
  }
