...
public class DumpMobSkills
{
  ...
  public DumpMobSkills(boolean update) throws Exception {
    this.update = update;
    this.skill = MapleDataProviderFactory.getDataProvider(new File(new StringBuilder().append(System.getProperty("net.sf.odinms.wzpath")).append("/Skill.wz").toString()));
    if (this.skill == null)
      this.hadError = true;
  }
  ...
  public void dumpMobSkills(PreparedStatement ps) throws Exception
  {
    ...
    for (Iterator i$ = skillz.getChildren().iterator(); i$.hasNext(); ) { ids = (MapleData)i$.next();
      for (MapleData lvlz : ids.getChildByPath("level").getChildren()) {
        this.id = Integer.parseInt(ids.getName());
        int lvl = Integer.parseInt(lvlz.getName());
