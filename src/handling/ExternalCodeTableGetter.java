...
public class ExternalCodeTableGetter {
    ...
    private final <T extends Enum<? extends WritableIntValueHolder> & WritableIntValueHolder> int getValue(final String name, T[] values, final int def) {
        ...
        if (prop != null && prop.length() > 0) {
            ...
            if (offset.length() > 2 && offset.substring(0, 2).equals("0x")) {
                return (int) (Integer.parseInt(offset.substring(2), 16) + base);
            } else {
                return (int) (Integer.parseInt(offset) + base);
            }
        }
        return def;
    }
    ...
    public final static <T extends Enum<? extends WritableIntValueHolder> & WritableIntValueHolder> void populateValues(Properties properties, T[] values) {
        ...
        for (T code : values) {
            code.setValue(exc.getValue(code.name(), values, (int) -2));
        }
