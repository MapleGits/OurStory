...
public class ItemMakerHandler {
        ...
        switch (makerType) {
            case 1:
                ...
                if (GameConstants.isGem(toCreate)) {
                    ...
                    c.getSession().write(CField.EffectPacket.ItemMaker_Success());
                    c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.EffectPacket.ItemMaker_Success_3rdParty(c.getPlayer().getId()), false);
    ...
    public static final void UseBag(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (firstTime) {
            ...
            toUse.setFlag(flag);
            c.getSession().write(CWvsContext.InventoryPacket.updateSpecialItemUse(toUse, (byte) 4, toUse.getPosition(), true, chr));
        }
        c.getSession().write(CField.openBag(chr.getExtendedSlots().indexOf(Integer.valueOf(itemId)), itemId, firstTime));
        c.getSession().write(CWvsContext.enableActions());
    }
    ...
    public static final void ProfessionInfo(LittleEndianAccessor slea, MapleClient c) {
        try {
    ...
    public static final void UsePot(LittleEndianAccessor slea, MapleClient c) {
        int itemid = slea.readInt();
        ...
        for (int i = 0; i < c.getPlayer().getImps().length; i++) {
            if (c.getPlayer().getImps()[i] == null) {
                ...
                c.getSession().write(CWvsContext.updateImp(c.getPlayer().getImps()[i], MapleImp.ImpFlag.SUMMONED.getValue(), i, false));
    ...
    public static final void ClearPot(LittleEndianAccessor slea, MapleClient c) {
        int index = slea.readInt() - 1;
        if ((index < 0) || (index >= c.getPlayer().getImps().length) || (c.getPlayer().getImps()[index] == null)) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        c.getSession().write(CWvsContext.updateImp(c.getPlayer().getImps()[index], MapleImp.ImpFlag.REMOVED.getValue(), index, false));
        c.getPlayer().getImps()[index] = null;
    }

    public static final void FeedPot(LittleEndianAccessor slea, MapleClient c) {
        int itemid = slea.readInt();
    ...
    public static final void CurePot(LittleEndianAccessor slea, MapleClient c) {
        int itemid = slea.readInt();
        ...
        c.getPlayer().getImps()[index].setState(1);
        c.getSession().write(CWvsContext.updateImp(c.getPlayer().getImps()[index], MapleImp.ImpFlag.STATE.getValue(), index, false));
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.ETC, slot.getPosition(), (short) 1, false, false);
    }

    public static final void RewardPot(LittleEndianAccessor slea, MapleClient c) {
        int index = slea.readInt() - 1;
