...
import static clientside.BuddyList.BuddyOperation.ADDED;
import static clientside.BuddyList.BuddyOperation.DELETED;
...
public class BuddyListHandler {
    ...
    public static void BuddyOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        // c.getPlayer().dropMessage(5, "Mode: " + mode);
        if (mode == 1) { // add
            ...
            if (!buddylist.isFull() && ble != null && !ble.isVisible()) {
                ...
                buddylist.put(new BuddylistEntry(ble.getName(), otherCid, "Group Unknown", channel, true));
                c.getSession().write(BuddylistPacket.updateBuddylist(buddylist.getBuddies(), 10));
                notifyRemoteChannel(c, channel, otherCid, "Group Unknown", ADDED);
            } else {
                c.getSession().write(BuddylistPacket.buddylistMessage((byte) 11));
            }
