...
function action(mode, type, selection) {
	...
	var time = parseInt(data);
        ...
        cm.sendSimple("#b#L100#Crimson Queen#l");
    }else if (status == 1) {
        if (mode == 1) {
            switch (selection) {
                case 100:
                    if (cm.getParty() != null) {
                        if (cm.getDisconnected("QueenBattle") != null) {
                            cm.getDisconnected("QueenBattle").registerPlayer(cm.getPlayer());
                        } else if (cm.isLeader()) {
                            ...
                            while (it.hasNext()) {
										    var i = 0;
        var full = true;
        if (cm.getMap(105200710 + i).getCharactersSize() == 0 && cm.getMap(105200710 + i) !=null) {
								            i++;
            if (cm.getMap(105200710 + i).getCharactersSize())
                full = true;
				else
				full = false;
        }
            ...
            return;
                            ...
                            if (next) {
                                var q = cm.getEventManager("QueenBattle");
                                if (q == null) {
